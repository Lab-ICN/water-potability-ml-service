...
for i in {1..10}; do
  if mc alias set minio http://localhost:443 "${MINIO_ROOT_USER}" "${MINIO_ROOT_PASSWORD}" &>/dev/null; then
...
if mc mb --ignore-existing minio/mlflow; then
...
if mc mb --with-versioning --ignore-existing minio/dataset; then
